...
export class DuplicateDetectionService {
  ...
  async detectDuplicates(leadData: {
    ...
    if (leadData.phone)
      OR_CONDITIONS.push({ contact: { phone: leadData.phone } });
    if (leadData.panNumber)
      OR_CONDITIONS.push({ contact: { panNumber: leadData.panNumber } });
    if (leadData.aadhaarNumber)
      OR_CONDITIONS.push({
        contact: { aadhaarNumber: leadData.aadhaarNumber },
      });
    if (leadData.gstNumber)
      OR_CONDITIONS.push({ contact: { gstNumber: leadData.gstNumber } });
