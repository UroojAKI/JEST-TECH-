...
export class LeadCompletionService {
  ...
  async computeCompletionStatus(leadId: string): Promise<LeadCompletionStatus> {
    ...
    if (
      ...
      (lead.description &&
        /previous policy|expiring|ncb/i.test(lead.description))
    ) {
      stage4Score = 20;
    } else {
      stage4Missing.push(
      ...
      blockingReasons.push(
        'Previous policy/NCB evidence is required before quotation readiness.',
      );
    }
    stages.push({
      stage: 4,
    ...
    const isQualifiedForQuotation =
      totalScore >= 60 && stages.every((stage) => stage.isComplete);
