...
export class AccountsService {
  ...
  private scope(actor: ActorContext): Prisma.AccountWhereInput {
    if (!actor?.userId || !actor.organizationId)
      throw new BadRequestException('Actor organizational context is required');
    ...
    const orgScope = {
      createdBy: {
        branch: { zone: { region: { companyId: actor.organizationId } } },
      },
    };
  ...
  async create(
    ...
    if (
      dto.gstNumber &&
      (await this.accountRepository.findByGstNumber(dto.gstNumber))
    )
    ...
    if (
      dto.panNumber &&
      (await this.accountRepository.findByPanNumber(dto.panNumber))
    )
      throw new ConflictException(
  ...
  async findAll(pagination: PaginationDto, actor: ActorContext) {
    ...
    const searchWhere: Prisma.AccountWhereInput = search
      ...
      : {};
    const where: Prisma.AccountWhereInput = Object.keys(scope).length
      ? { AND: [searchWhere, scope] }
      : searchWhere;
  ...
  async update(
    ...
    const scope = this.scope(actor);
    const scoped = await this.accountRepository.findAll(
      { AND: [{ id }, scope] },
      0,
      1,
    );
    ...
    if (
      dto.panNumber &&
      dto.panNumber !== existing.panNumber &&
      (await this.accountRepository.findByPanNumber(dto.panNumber))
    )
      throw new ConflictException(
        `An account with PAN number ${dto.panNumber} already exists`,
    ...
    if (
      ...
      (await this.accountRepository.findByName(dto.name))
    )
      this.logger.warn(
        `Potential duplicate account name detected: "${dto.name}"`,
      );
  ...
  async remove(id: string, deletedById: string, actor: ActorContext) {
    ...
    if (!roles.some((r) => GLOBAL_ROLES.includes(r)))
      throw new BadRequestException('Only administrators can delete accounts');
