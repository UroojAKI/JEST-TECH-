...
export class DocumentsController {
  ...
  @Get() async getAllDocuments(
    @Query() pagination: PaginationDto,
    @CurrentUser() user: RequestUser,
  ) {
    return this.documentService.findAll(pagination, user);
  }
  @Post('upload')
  ...
  @Get(':id') async getDocumentDetails(
    @Param('id', ParseUUIDPipe) id: string,
    @CurrentUser() user: RequestUser,
  ) {
    return this.documentService.getDocumentDetails(id, user);
  }
  @Delete(':id') async deleteDocument(
    @Param('id', ParseUUIDPipe) id: string,
    @CurrentUser() user: RequestUser,
    @Ip() ipAddress: string,
  ) {
    await this.documentService.softDeleteDocument(id, user.id, ipAddress, user);
    return { success: true, message: 'Document soft-deleted' };
  }
  @Post(':id/restore') async restoreDocument(
    @Param('id', ParseUUIDPipe) id: string,
    @CurrentUser() user: RequestUser,
    @Ip() ipAddress: string,
  ) {
    await this.documentService.restoreDocument(id, user.id, ipAddress, user);
    return { success: true, message: 'Document restored' };
  }
  @Get(':id/history') async getDocumentHistory(
    @Param('id', ParseUUIDPipe) id: string,
    @CurrentUser() user: RequestUser,
  ) {
    return this.documentService.getAccessLogs(id, user);
  }
  @Get(':id/download') async downloadDocument(
    @Param('id', ParseUUIDPipe) id: string,
    @CurrentUser() user: RequestUser,
    @Ip() ipAddress: string,
    @Res() res: Response,
    ...
    const { fileBuffer, originalFileName, mimeType } =
      await this.documentService.downloadDocument(id, user.id, ipAddress, user);
    const safe = path
  ...
  @Get(':id/verification') async getVerificationStatus(
    ...
    await this.documentService.getDocumentDetails(id, user);
