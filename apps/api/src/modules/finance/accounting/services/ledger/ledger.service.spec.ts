...
describe('LedgerService', () => {
  ...
  describe('postEntry', () => {
    it('should throw if debits and credits do not balance', async () => {
      ...
      await expect(service.postEntry(data)).rejects.toThrow(BadRequestException);
      await expect(service.postEntry(data)).rejects.toThrow('Journal entry is unbalanced');
    });
    ...
    it('should create journal entry if balanced', async () => {
      ...
      jest.spyOn(prisma.journalEntry, 'create').mockResolvedValue({ id: 'je-1' } as any);
  ...
  describe('getAccountBalance', () => {
    it('should calculate ASSET balance as Debit - Credit', async () => {
      jest.spyOn(prisma.chartOfAccount, 'findUnique').mockResolvedValue({ type: 'ASSET' } as any);
    ...
    it('should calculate LIABILITY balance as Credit - Debit', async () => {
      jest.spyOn(prisma.chartOfAccount, 'findUnique').mockResolvedValue({ type: 'LIABILITY' } as any);
      jest.spyOn(prisma.journalLine, 'aggregate').mockResolvedValue({
