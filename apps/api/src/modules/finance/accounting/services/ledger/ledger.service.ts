...
export class LedgerService {
   ...
   * Enforces that Total Debits == Total Credits and that every account exists.
   * Journal numbering is generated by a PostgreSQL sequence so concurrent posts
   * cannot produce duplicate entry numbers.
   */
  async postEntry(data: CreateJournalEntryDto): Promise<JournalEntry> {
    ...
    if (!data.description?.trim()) {
      throw new BadRequestException('Journal entry description is required.');
    }

    const entryDate = new Date(data.date);
    if (Number.isNaN(entryDate.getTime())) {
      throw new BadRequestException('Journal entry date is invalid.');
    }
    ...
    if (totalDebit.lte(0) || totalCredit.lte(0)) {
      throw new BadRequestException(
        'A journal entry must contain at least one debit and one credit.',
      );
    }
    ...
    if (missingAccounts.length) {
      throw new BadRequestException(
        `Unknown chart-of-account ID(s): ${missingAccounts.join(', ')}`,
      );
    }

    // PostgreSQL sequence is the authoritative source for journal numbers.
    // Gaps are acceptable for accounting sequence numbers; collisions are not.
