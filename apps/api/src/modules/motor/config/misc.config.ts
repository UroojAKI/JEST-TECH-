...
export const miscConfig: VehicleCategoryConfig = {
  ...
  fields: [
    { name: 'equipmentType', label: 'Equipment Sub-Classification', type: 'select', required: true, options: [
    ...
    { name: 'operatingEnvironment', label: 'Operating Area', type: 'select', required: true, options: [
      { label: 'Confined Site / Factory Premises Only', value: 'SITE_ONLY' },
      { label: 'Public Roads and Sites', value: 'PUBLIC_ROADS' },
    ]},
  ],
  allowedAddons: [
    { code: 'LEGAL_LIABILITY_OPERATOR', name: 'Legal Liability to Equipment Operator / Helper', description: 'Covers crane operator or certified machine driver', defaultSelected: true },
    { code: 'TOWING_OVERSIZED', name: 'Oversized Heavy Equipment Towing Cover', description: 'Specialized low-bed recovery' },
  ],
  requiredDocuments: {
    newVehicle: ['Equipment Tax Invoice', 'Manufacturer Specification Sheet', 'Factory Gate Pass'],
    existingComprehensive: ['Equipment Registration / Serial RC', 'Factory Inspector Certificate'],
    breakInInspection: ['Physical Surveyor Machinery Breakdown & Condition Report'],
