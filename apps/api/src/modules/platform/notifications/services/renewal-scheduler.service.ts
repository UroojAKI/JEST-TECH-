...
export class RenewalScheduler {
  ...
  async runRenewalScheduler() {
    ...
    this.logger.log(
      `Renewal scheduler: ${tasksCreated} new tasks created across ${policies.length} expiring policies.`,
    );
  }
  ...
  async handleEscalations() {
    ...
    for (const task of overdueTasks) {
      ...
      const escalateToId =
        task.agent?.managerId || (await this.getDefaultAgentId());
      if (!escalateToId) continue;

      await this.dispatcher.dispatch({
        userId: escalateToId,
        type: 'POLICY_RENEWAL_30', // Escalation reuses renewal notification type
        priority: 'HIGH',
    ...
    if (overdueTasks.length > 0) {
      this.logger.warn(
        `Escalated ${overdueTasks.length} overdue renewal tasks.`,
      );
    }
  }
