...
const contactOwnerInclude = {
  createdBy: { include: { branch: true, team: true } },
} as const;
...
export class ContactRepository {
  ...
  async update(id: string, data: Prisma.ContactUpdateInput): Promise<ContactWithOwner> {
    return this.prisma.contact.update({ where: { id }, data, include: contactOwnerInclude });
  }

  async softDelete(id: string, deletedById: string): Promise<Contact> {
    return this.prisma.contact.update({ where: { id }, data: { deletedAt: new Date(), updatedById: deletedById } });
  }
}
