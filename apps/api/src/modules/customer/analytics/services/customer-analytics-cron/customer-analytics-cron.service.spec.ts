...
describe('CustomerAnalyticsCronService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<CustomerAnalyticsCronService>(CustomerAnalyticsCronService);
    prisma = module.get<PrismaService>(PrismaService);
  });

  describe('calculateMetricsForCustomer', () => {
    it('should correctly aggregate premiums and calculate LTV', async () => {
      jest.spyOn(prisma.policy, 'count').mockImplementation(async ({ where }: any) => {
      ...
      jest.spyOn(prisma.claim, 'findMany').mockResolvedValue([
        { amount: new Decimal(5000) }
      ] as any);

      jest.spyOn(prisma.customerAnalytics, 'upsert').mockResolvedValue({} as any);
    ...
    it('should lower renewal probability for high claims ratio', async () => {
      ...
      jest.spyOn(prisma.policy, 'findMany').mockResolvedValue([{ premiumAmount: new Decimal(10000) }] as any);
      jest.spyOn(prisma.claim, 'findMany').mockResolvedValue([{ amount: new Decimal(15000) }] as any); // 150% ratio
