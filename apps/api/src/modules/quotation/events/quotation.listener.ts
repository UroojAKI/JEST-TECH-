...
export class QuotationListener {
  ...
  async handleLeadConvertedEvent(event: LeadConvertedEvent) {
    const { lead } = event;
    this.logger.log(
      `Processing lead.converted event for lead code: ${lead.leadCode}`,
    );
    ...
    try {
      ...
      const quotation = await this.generateQuotationService.execute(
        draftDto,
        creatorId,
      );
      this.logger.log(
        `Automatically generated draft quotation ${quotation.quotationCode} for lead ${lead.leadCode}`,
      );
    } catch (error: any) {
      this.logger.error(
