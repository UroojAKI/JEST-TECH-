...
export abstract class BaseRepository<ModelDelegate, BasicType, DetailType> {
  ...
  async findById(id: string, tx?: TransactionClient): Promise<DetailType | null> {
  ...
  async findBasicById(id: string, tx?: TransactionClient): Promise<BasicType | null> {
    return (this.getClient(tx) as any).findFirst({
      where: { id, deletedAt: null },
      ...this.basicArgs,
    });
  ...
  async createMany(data: any[], tx?: TransactionClient): Promise<{ count: number }> {
  ...
  async update(id: string, data: any, tx?: TransactionClient): Promise<DetailType> {
   ...
   */
  async softDelete(id: string, deletedById?: string, tx?: TransactionClient): Promise<void> {
