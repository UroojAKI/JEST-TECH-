...
export class ResourceAuthorizationService {
  authorize(actor: ActorContext, resourceType: ResourceType, action: ResourceAction, resource?: any): boolean {
    ...
    if (roles.some((r) => GLOBAL_ROLES.includes(r)) || actor.permissions?.includes('*')) return true;
    switch (action) {
  ...
  canRead(actor: ActorContext, resource: any, resourceType: ResourceType): boolean {
    ...
    const roles = actor.roles?.length ? actor.roles : [actor.role];
    if (roles.some((r) => GLOBAL_ROLES.includes(r) || ORGANIZATION_ADMIN_ROLES.includes(r))) return true;
    if (roles.some((r) => BRANCH_ROLES.includes(r))) { const branchId = this.resourceBranchId(resource); if (!actor.branchId || !branchId) throw new ForbiddenException('Branch context is required for branch-scoped access'); if (actor.branchId !== branchId) throw new ForbiddenException('Resource belongs to another branch'); return true; }
    if (roles.some((r) => TEAM_ROLES.includes(r))) { const teamId = this.resourceTeamId(resource); if (!actor.teamId || !teamId) throw new ForbiddenException('Team context is required for team-scoped access'); if (actor.teamId !== teamId) throw new ForbiddenException('Resource belongs to another sales team'); return true; }
  ...
  canApprove(actor: ActorContext, resourceType: ResourceType): boolean {
    const roles = actor.roles?.length ? actor.roles : [actor.role];
    if (roles.some((r) => [...GLOBAL_ROLES, ...ORGANIZATION_ADMIN_ROLES].includes(r))) return true;
