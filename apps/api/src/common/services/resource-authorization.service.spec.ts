...
describe('ResourceAuthorizationService & ScopeResolver', () => {
  ...
  const createActor = (overrides: Partial<ActorContext>): ActorContext => ({
    userId: 'usr-agent-a', email: 'agent.a@jest.com', firstName: 'Agent', lastName: 'A',
    organizationId: 'org-mumbai', companyId: 'org-mumbai', branchId: 'branch-andheri', branchCode: 'ANDHERI',
    departmentId: 'dept-sales', teamId: 'team-motor-a', role: RoleType.SALES_AGENT,
    roles: [RoleType.SALES_AGENT], permissions: ['quotation.read', 'quotation.create'], workspaces: ['SALES'], status: UserStatus.ACTIVE,
    ...overrides,
  });
  ...
  it('allows an agent to read an owned quotation', () => {
    ...
    expect(authzService.authorize(actor, 'QUOTATION', 'READ', { id: 'quote-1', createdById: 'usr-agent-a', ...resourceOrg })).toBe(true);
  });

  it('rejects an agent reading another agent quotation', () => {
    const actor = createActor({ userId: 'usr-agent-a' });
    expect(() => authzService.authorize(actor, 'QUOTATION', 'READ', { id: 'quote-2', createdById: 'usr-agent-b', ...resourceOrg })).toThrow(ForbiddenException);
  });
  ...
  it('allows a team leader to access records in their team and rejects another team', () => {
    ...
    expect(authzService.authorize(actor, 'QUOTATION', 'READ', { id: 'quote-3', createdById: 'usr-agent-a', ...resourceOrg, createdBy: { ...resourceOrg.createdBy, teamId: 'team-a' } })).toBe(true);
    expect(() => authzService.authorize(actor, 'QUOTATION', 'READ', { id: 'quote-4', createdById: 'usr-agent-b', ...resourceOrg, createdBy: { ...resourceOrg.createdBy, teamId: 'team-b' } })).toThrow(ForbiddenException);
  });
  ...
  it('allows a branch manager only inside their branch', () => {
    ...
    const inBranch = { ...resourceOrg, createdBy: { ...resourceOrg.createdBy, branchId: 'branch-a' } };
    const outBranch = { ...resourceOrg, createdBy: { ...resourceOrg.createdBy, branchId: 'branch-b' } };
    expect(authzService.authorize(actor, 'QUOTATION', 'READ', { id: 'quote-5', createdById: 'usr-agent-a', ...inBranch })).toBe(true);
    expect(() => authzService.authorize(actor, 'QUOTATION', 'READ', { id: 'quote-6', createdById: 'usr-agent-b', ...outBranch })).toThrow(ForbiddenException);
  });

  it('rejects cross-tenant access before role shortcuts', () => {
    const actor = createActor({ organizationId: 'org-mumbai' });
    expect(() => authzService.authorize(actor, 'QUOTATION', 'READ', { id: 'quote-7', organizationId: 'org-delhi', createdById: 'usr-agent-z' })).toThrow(ForbiddenException);
  });
  ...
  it('requires organization context even for super admin', () => {
    ...
    expect(() => authzService.authorize(actor, 'QUOTATION', 'READ', { id: 'quote-8', createdById: 'usr-agent-z' })).toThrow(ForbiddenException);
  });
  ...
  it('rejects suspended and inactive users', () => {
    for (const status of [UserStatus.SUSPENDED, UserStatus.INACTIVE]) {
      ...
      expect(() => authzService.authorize(actor, 'QUOTATION', 'READ', { id: 'quote-9', createdById: actor.userId, ...resourceOrg })).toThrow(ForbiddenException);
    }
  });
  ...
  it('creates ownership scope for sales agents', () => {
    const actor = createActor({ userId: 'usr-agent-a' });
    expect(scopeResolver.resolveScopeFilter(actor, 'QUOTATION')).toEqual({ OR: [{ createdById: 'usr-agent-a' }, { lead: { assignedToId: 'usr-agent-a' } }] });
  });
  ...
  it('creates organization-scoped filters for operational roles', () => {
    ...
    expect(scopeResolver.resolveScopeFilter(actor, 'LEAD')).toEqual({ OR: [
  ...
  it('fails closed for an actor without organization context', () => {
    ...
    expect(scopeResolver.resolveScopeFilter(actor, 'QUOTATION')).toEqual({ id: '__UNAUTHORIZED_ACCESS_BLOCKED__' });
  });
});
