...
export default function CustomerRegisterPage() {
  const router = useNav();
  const [savedView, setSavedView] = useState<'ALL' | 'CORPORATE'>('ALL');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ firstName: '', lastName: '', type: 'INDIVIDUAL', phone: '', email: '' });
    } catch (err: any) {
  ...
  const visibleCustomers = savedView === 'CORPORATE' ? customers.filter((c: any) => c.type === 'CORPORATE') : customers;
  const columns = [
  ...
  return (
    <AppShell>
      ...
      {isLoading ? <div className="p-10 flex justify-center"><Loader2 className="h-5 w-5 animate-spin" /></div> : <EnterpriseTable data={visibleCustomers} columns={columns} totalRows={savedView === 'ALL' ? total : visibleCustomers.length} pageSize={limit} pageIndex={page - 1} pageCount={savedView === 'ALL' ? totalPages : 1} manualPagination manualFiltering searchValue={search} onSearchChange={(value) => { setSearch(value); setPage(1); }} onPageChange={(nextPage) => setPage(nextPage + 1)} />}
    </AppShell>
  );
}
