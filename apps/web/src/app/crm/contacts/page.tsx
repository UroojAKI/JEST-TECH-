...
export default function CustomerRegisterPage() {
  ...
  const limit = 25;
  const tag = savedView === 'VIP' ? 'VIP' : savedView === 'CORPORATE' ? 'CORPORATE' : undefined;

  const { customers, total, totalPages, isLoading, isError, refetch } = useCustomers({ page, limit, search: search.trim() || undefined, tag, sortBy: 'createdAt', sortOrder: 'desc' });

  const [formData, setFormData] = useState({ firstName: '', lastName: '', type: 'INDIVIDUAL', phone: '', email: '', tag: 'REGULAR' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await customerRepository.createContact({ type: formData.type as 'INDIVIDUAL' | 'CORPORATE', firstName: formData.firstName.trim(), lastName: formData.lastName.trim(), phone: cleanPhone, email: formData.email.trim() || undefined, tags: formData.tag ? [formData.tag] : [] });
      await refetch();
      toast.success(`Customer "${formData.firstName} ${formData.lastName}" registered successfully`);
      setShowAddModal(false); setPage(1);
  ...
  const columns = [
    ...
    { accessorKey: 'tags', header: 'Tags', cell: ({ row }: any) => row.original.tags?.length ? <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-primary/10 text-primary border border-primary/20">{row.original.tags.join(', ')}</span> : <span>—</span> },
