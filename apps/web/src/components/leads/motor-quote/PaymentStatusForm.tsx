    ...
    <div className="space-y-6">
      {totalPremium && totalPremium > 0 && (
        <div className="p-4 rounded-2xl border bg-primary/5 text-center">
          ...
          <div className="text-[10px] text-muted-foreground">Authoritative backend quotation amount</div>
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        {PAYMENT_OPTIONS.map(({ value: v, label, description, icon, color }) => (
          <button
            ...
            className={`w-full flex items-center gap-4 p-4 rounded-2xl border text-left transition-all ${value.status === v ? `bg-${color}-500/10 border-${color}-400 text-${color}-700` : 'bg-background hover:bg-accent border-border'}`}
          >
            <div className={value.status === v ? `text-${color}-600` : 'text-muted-foreground'}>{icon}</div>
            <div><div className="font-extrabold text-sm">{label}</div><div className="text-[11px] text-muted-foreground">{description}</div></div>
            <div className={`ml-auto w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${value.status === v ? `border-${color}-600 bg-${color}-600` : 'border-border'}`}>
      ...
      {(value.status === 'PAID' || value.status === 'UNDER_PROCESS') && (
        <div className="space-y-3 p-4 rounded-2xl border bg-card">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {value.status === 'PAID' && (
              <div>
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">Amount Received (₹)</label>
                <input
                  required
                  type="number"
                  min="0.01"
                  step="0.01"
                  value={value.amount ?? totalPremium ?? ''}
            ...
            <div>
              ...
              <select value={value.paymentMethod || ''} onChange={(e) => update({ paymentMethod: e.target.value })} className="w-full mt-1 px-3 py-2 rounded-xl border bg-background text-sm">
            ...
            <div>
              ...
              <input type="text" value={value.referenceNumber || ''} onChange={(e) => update({ referenceNumber: e.target.value })} placeholder="Transaction reference" className="w-full mt-1 px-3 py-2 rounded-xl border bg-background text-sm" />
            </div>
            {value.status === 'PAID' && (
              <div>
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">Payment Date</label>
                <input type="date" value={value.paidAt || ''} onChange={(e) => update({ paidAt: e.target.value })} className="w-full mt-1 px-3 py-2 rounded-xl border bg-background text-sm" />
              </div>
            )}
            <div className="sm:col-span-2">
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">Notes</label>
              <textarea value={value.notes || ''} onChange={(e) => update({ notes: e.target.value })} rows={2} placeholder="Optional notes" className="w-full mt-1 px-3 py-2 rounded-xl border bg-background text-sm resize-none" />
            </div>
          </div>
        </div>
      )}
      ...
      {value.status === 'PAID' && (
        <div className="p-3 rounded-xl bg-emerald-500/10 text-emerald-700 text-xs font-bold text-center">
          ✓ Payment confirmed — quotation is now waiting for Back Office policy issuance.
        </div>
      )}
    </div>
  );
