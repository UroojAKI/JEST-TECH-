...
export class MotorQuotationMigrationService {
  ...
  async deduplicateMotorPolicies(isDryRun = false): Promise<number> {
    ...
    const duplicates = await this.prisma.$queryRaw<
      Array<{ quotation_id: string; policy_count: bigint | number }>
    >`
    ...
    for (const dup of duplicates) {
      ...
      const canonicalPolicy =
        policies.find((p) => p.status === 'ISSUED') || policies[0];

      const redundantPolicies = policies.filter((p) => p.id !== canonicalPolicy.id);
  ...
  async assertZeroOrphans(): Promise<{ pass: boolean; orphans: Record<string, number> }> {
    ...
    // 1. Orphan proposals
    const orphanProposals = await this.prisma.$queryRaw<Array<{ count: bigint | number }>>`
    ...
    // 2. Orphan payments
    const orphanPayments = await this.prisma.$queryRaw<Array<{ count: bigint | number }>>`
    ...
    const orphanInspections = await this.prisma.$queryRaw<Array<{ count: bigint | number }>>`
  ...
  async executeMigration(isDryRun = false): Promise<MigrationSummary> {
    ...
    const legacyMotorQuotes = await this.prisma.motorQuotation.findMany({
    ...
    for (const legacy of legacyMotorQuotes) {
      ...
      let canonical = await this.prisma.quotation.findFirst({
        where: { quotationCode: legacy.quotationNumber },
      });

      if (!canonical) {
        if (!isDryRun) {
          canonical = await this.prisma.quotation.create({
            data: {
    ...
    const postDupQuery = await this.prisma.$queryRaw<Array<{ count: bigint | number }>>`
...
if (require.main === module) {
  ...
  const migration = new MotorQuotationMigrationService(prisma);
  migration
